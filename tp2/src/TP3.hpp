 ...
 * @param selected
 */
inline void drawItem(Icon icon, string name, int x, int y, bool selected = false) {
...
inline int getIndex(const int &x, const int &y) {
    int index = x / Window::getIconWidth() + (y / Window::getIconHeight() * Window::getWidth() / Window::getIconWidth());
 ...
 */
inline void onWindowClick(const int &x, const int &y, const bool &button, const bool &ctrl) {
